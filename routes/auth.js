...
router.post(
  ...
  async function (req, res, next) {
    try {
      ...
      responseHandler.CreateErrorResponse(res, 500, "Lỗi khi tạo tài khoản");
      //   next(error);
    }
  }
);
...
router.post(
  ...
  function (req, res, next) {
    try {
      ...
      let result = userController.Change_Password(
        req.user,
        oldpassword,
        newpassword
      );
...
router.post("/forgotpassword", async function (req, res, next) {
  try {
    ...
    await user.save();
    let url =
      "http://localhost:3000/auth/resetpassword/" + user.resetPasswordToken;
